...
<script type="text/javascript">
	var sysBBS = "<span style='font-size:14px; line-height:30px;'>欢迎光临心之语聊天室，请遵守聊天室规则，不要使用不文明用语。</span><br></span><br><span style='line-height:22px;'>";
	...
	function check() {
		...
		$.post("${pageContext.request.contextPath}/user?method=check",
				function(data) {
					//返回的字符串如果是 still in表示登录的。 如果是 be kicked out 表示被踢出了
					if (data != "still in") {
						// 提示用户下线了
						alert("用户已经被踢下线了!");
						// 回到登录页面!
	...
	function send() {
		...
		$.post("${pageContext.request.contextPath}/user?"
				+ new Date().getTime(), $("#form1").serialize(),function(data){
					//如果返回 success 表示发送成功了  清空$("#content")
					if(data == "success")
						form1.content.value="";
				});
	}
	...
	function checkScrollScreen() {
		...
		if ($("#scrollScreen").attr("checked")) {
			...
			//当聊天信息超过一屏时，设置最先发送的聊天信息不显示。 这里设置滚动的位置距离top是内容高度的两倍  这样保证了滚动条永远都在最下面。
	...
	<table width="778" height="95" border="0" align="center"
		...
		<form action="" id="form1" name="form1" method="post">
			...
			<tr>
				...
				<td height="37" align="left"><input name="from" type="hidden"
					value="${user.username}">[${user.username} ]对 <!-- 聊天对象，只能在列表中选择，这个文本框设置位readonly，只能在代码中用js替换 -->
				...
				<td width="19" align="left"><input name="scrollScreen"
					...
					onClick="checkScrollScreen()" value="1"></td>
			</tr>
			<tr>
				<td width="21" height="30" align="left">&nbsp;</td>
				<td width="549" align="left"><input id="content" name="content" type="text"
					size="70"
				...
				<td align="right"><input name="button_exit" type="button"
					class="btn_grey" value="退出聊天室" onClick="exit()"></td>
				<td align="center">&nbsp;</td>
			</tr>
			<tr>
				<td height="30" align="left">&nbsp;</td>
